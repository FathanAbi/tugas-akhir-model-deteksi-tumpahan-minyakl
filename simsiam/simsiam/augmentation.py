...
def Augment_data (x_train,y_train, n_category, n_patch, n_band, num_per_category ):
    ...
    for i in range(number):
        ...
        # rotasi 90 derajat + flip sumbu x
        if need>0:   
        ...
        # rotasi 90 derajat + flip sumbu y
        if need> 0:
...
def Augment_data2 (x_train,y_train, n_category, n_patch, n_band, num_per_category):
    '''
    ...
    n_copy = math.ceil((num_per_category - nlabeled) / nlabeled) + 1
    #rotation 
    k = 0
    for i in range(number):
